...
function validateDefinition(item: unknown): boolean {
  const { sectionNumber, name, elementList, rangeBounds, conditions } =
    item as Definition;
...
export class Definitions {
  ...
  public static fromObject(obj: unknown) {
    ...
    const definitionList = definitionListObj.map((definitionObj) =>
      Definition.fromObject(definitionObj)
    );
    return new Definitions(definitionList);
  }

  public findDefinition(sectionNumberOrName: string): Definition | undefined {
    const definition = this.definitionList.find(
      (def) =>
